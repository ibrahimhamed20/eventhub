...
@Component({
  ...
  template: `
    <div class="min-h-[calc(100vh-8rem)] flex items-center justify-center px-4 py-12">
      <div class="w-full max-w-md space-y-8 glass-panel p-8 rounded-2xl border border-slate-800 shadow-2xl">
        <!-- Header -->
        <div class="text-center space-y-2">
          ...
          <p class="text-sm text-slate-400">Sign in to access your bookings and manage events</p>
        </div>
            ...
            <div class="flex-1">
              ...
              @if (authStore.rateLimitCooldownSec(); as cooldown) {
                <p class="text-xs text-rose-300/80 mt-1 font-mono">
                  Cooldown active: please retry in {{ cooldown }}s
                </p>
              }
            </div>
          </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-6" novalidate>
          ...
          <button
            ...
            class="w-full py-3 px-4 rounded-xl font-semibold text-sm text-white bg-indigo-600 hover:bg-indigo-500 active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none transition-all shadow-lg shadow-indigo-600/25 flex items-center justify-center gap-2"
          >
            @if (isSubmitting()) {
        ...
        <div class="text-center pt-2">
          <p class="text-sm text-slate-400">
            ...
            <a routerLink="/register" class="font-medium text-indigo-400 hover:text-indigo-300 hover:underline">
              Create account
            </a>
          </p>
        </div>
